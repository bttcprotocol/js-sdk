...
export default class TronExitManager extends TronContractsBase {
  ...
  async buildPayloadForExit(burnTxHash, logEventSig, requestConcurrency?) {
    ...
    const contract = await this.rootChain.rootChain.tronWeb.contract(
      this.rootChain.rootChain.abi,
      this.rootChain.rootChain.token
    )
    const headerBlock = await this.web3Client.call(contract.methods.headerBlocks(this.encode(headerBlockNumber)))
...
//     assert.ok(logIndex > -1, 'Log not found in receipt')
  ...
  async buildPayloadForExitHermoine(burnTxHash, logEventSig) {
    ...
    const receipt = await this.web3Client.getMaticWeb3().eth.getTransactionReceipt(burnTxHash)
    const lastChildBlockNumber = new BN(lastChildBlock.toNumber())
    const receiptBlockNumber = new BN(receipt.blockNumber)
    const block: any = await this.web3Client
      .getMaticWeb3()
    ...
    assert.ok(lastChildBlockNumber.gte(receiptBlockNumber), 'Burn transaction has not been checkpointed as yet')
    ...
    try {
      ...
      headerBlock = await this.web3Client.call(contract.methods.headerBlocks(this.encode(headerBlockNumber)))
    }
