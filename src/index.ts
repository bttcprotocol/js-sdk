...
import TronPOSRootChainManager from './root/TronPOSRootChainManager'
...
export class BttcPOSClient extends SDKClient {
...
export class TronWebClient extends TronSDKClient {
  private rootChain: TronRootChain
  private posRootChainManager: TronPOSRootChainManager
  private tronWebClientOptions: Object

  constructor(options: any = {}) {
    options.network = TronSDKClient.initializeNetwork(options.network, options.version)
    if (!options.rootChain) {
      options.rootChain = options.network.Main.Contracts.RootChainProxy
    }
    super(options)
    this.tronWebClientOptions = options
    this.rootChain = new TronRootChain(options, this.web3Client)
    this.posRootChainManager = new TronPOSRootChainManager(options, this.rootChain, this.web3Client)
  }
...
export default class Bttc extends SDKClient {
  ...
  public utils: Utils
  public static BttcPOSClient = BttcPOSClient // workaround for web compatibility
  public static TronWebClient = TronWebClient // workaround for web compatibility
