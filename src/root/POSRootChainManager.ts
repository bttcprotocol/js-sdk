...
export default class POSRootChainManager extends ContractsBase {
  ...
  async exitTron(burnTxHash: string, logSignature: string, options?: SendOptions, tronOptions?: Object) {
    if (!this.posRootChainManager.options.address) {
      throw new Error('posRootChainManager address not found. Set it while constructing MaticPOSClient.')
    }
    const payload = await this.exitManager.buildPayloadForExitTron(
      burnTxHash,
      logSignature,
  ...
  async exitTronERC20(burnTxHash: string, options?: SendOptions, tronOptions?: Object) {
    return this.exitTron(burnTxHash, ERC20_TRANSFER_EVENT_SIG, options, tronOptions)
  }
