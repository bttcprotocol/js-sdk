...
export default class POSRootChainManager extends TronContractsBase {
  public posRootChainManager: any //Tronweb.Contract //Contract
  private exitManager: TronExitManager
  ...
  constructor(options: MaticClientInitializationOptions, rootChain: TronRootChain, web3Client: TronWeb3Client) {
    ...
    this.exitManager = new TronExitManager(rootChain, options, web3Client)
  ...
  async getPredicateAddress(rootToken: address) {
    const contract = await this.posRootChainManager.tronWeb.contract(
      this.posRootChainManager.abi,
      this.posRootChainManager.token
    )
    // const tokenType = await this.posRootChainManager.methods.tokenToType(rootToken).call()
    const tokenType = await contract.methods.tokenToType(rootToken).call()
    if (!tokenType) {
      throw new Error('Invalid Token Type')
    }
    //const predicateAddress = await this.posRootChainManager.methods.typeToPredicate(tokenType).call()
    const predicateAddress = await contract.methods.typeToPredicate(tokenType).call()
  ...
  async depositEtherForUser(amount: BN | string, user: address, options: SendOptions = {}) {
    ...
    const txObject = await contract.methods.depositEtherFor(user)
  ...
  async depositFor(user: address, rootToken: address, depositData: string, options?: SendOptions) {
    ...
    const txObject = await contract.methods.depositFor(user, rootToken, depositData)
  ...
  async exitHermoine(burnTxHash: string, logSignature: string, options?: SendOptions) {
    ...
    // const txObject = this.posRootChainManager.methods.exit(payload)
    const txObject = contract.methods.exit(payload)
    const web3Options = await this.web3Client.fillOptions(txObject, true /* onRootChain */, options)
    if (web3Options.encodeAbi) {
      return Object.assign(web3Options, {
        data: txObject.encodeABI(),
  ...
  async isExitProcessed(burnTxHash: string, logSignature: string) {
    ...
    const process = await contract.methods.processedExits(exitHash).call()
    return process //this.posRootChainManager.methods.processedExits(exitHash).call()
  }
  ...
  async processReceivedMessage(contractAddress: address, txHash: string, options?: SendOptions) {
    ...
    const rootTunnelContract = await this.posRootChainManager.tronWeb.contract(
      this.rootTunnelContractAbi,
      contractAddress
    )

    //let rootTunnelContract = new this.web3Client.parentWeb3.eth.Contract(this.rootTunnelContractAbi, contractAddress)
    let txObject = await rootTunnelContract.methods.receiveMessage(payload)
    const web3Options = await this.web3Client.fillOptions(txObject, true /* onRootChain */, options)
    if (web3Options.encodeAbi) {
      return Object.assign(web3Options, {
        data: txObject.encodeABI(),
  ...
  async approveERC20(rootToken: address, amount: BN | string, options?: SendOptions) {
    ...
    const tronWebOptions = this.getPOSERC20TokenContract(rootToken, true)
    ...
    const txObject = await contract.methods.approve(predicate, this.formatUint256(amount))
    const web3Options = await this.web3Client.fillOptions(txObject, true /* onRootChain */, options)
    if (web3Options.encodeAbi) {
  ...
  async approveMaxERC20(rootToken: address, options?: SendOptions) {
    ...
    const txObject = await contract.methods.approve(
      predicate,
      '0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'
    )
    const web3Options = await this.web3Client.fillOptions(txObject, true /* onRootChain */, options)
  ...
  async allowanceOfERC20(userAddress: address, token: address, options?: SendOptions) {
    ...
    const tronWebOptions = this.getPOSERC20TokenContract(token, true)
    ...
    /*const allowance = await this.getPOSERC20TokenContract(token, true)
      ...
      .call()*/
    const allowance = await contract.methods.allowance(userAddress, predicate).call()
    return allowance
  }
  ...
  async exitTronERC20(burnTxHash: string, options?: SendOptions) {
  ...
  async approveERC721(rootToken: address, tokenId: BN | string, options?: SendOptions) {
    const predicate = await this.getPredicateAddress(rootToken)
    const tronWebOptions = this.getPOSERC721TokenContract(rootToken, true)
    const contractAddress = this.utils.transferAddress(this.posRootChainManager.tronWeb, rootToken, false)
    const contract = await tronWebOptions.web3.contract(tronWebOptions.abi, contractAddress)
    ...
    const txObject = await contract.methods.approve(predicate, this.formatUint256(tokenId))
    const web3Options = await this.web3Client.fillOptions(txObject, true /* onRootChain */, options)
    if (web3Options.encodeAbi) {
      return Object.assign(web3Options, { data: txObject.encodeABI(), to: rootToken })
    }
  ...
  async isApprovedERC721(token: address, tokenId: BN | string, options?: SendOptions) {
    ...
    const predicate = await this.getPredicateAddress(token)
    const tronWebOptions = this.getPOSERC721TokenContract(token, true)
    const contractAddress = this.utils.transferAddress(this.posRootChainManager.tronWeb, token, false)
    const contract = await tronWebOptions.web3.contract(tronWebOptions.abi, contractAddress)
  ...
  async approveAllERC721(rootToken: address, options?: SendOptions) {
    ...
    const txObject = await contract.methods.setApprovalForAll(predicate, true)
    const web3Options = await this.web3Client.fillOptions(txObject, true /* onRootChain */, options)
    if (web3Options.encodeAbi) {
      return Object.assign(web3Options, { data: txObject.encodeABI(), to: rootToken })
    }
  ...
  async isApprovedForAllERC721(token: address, userAddress: address, options?: SendOptions) {
    ...
    /*const approved = await this.getPOSERC721TokenContract(token, true)
      ...
      .call()*/
    const approved = await contract.methods.isApprovedForAll(userAddress, predicate).call()
    return approved
  }
  ...
  async approveERC1155(rootToken: address, options?: SendOptions) {
    const predicate = await this.getPredicateAddress(rootToken)
    const tronWebOptions = this.getPOSERC1155TokenContract(rootToken, true)
    const contractAddress = this.utils.transferAddress(this.posRootChainManager.tronWeb, rootToken, false)
    const contract = await tronWebOptions.web3.contract(tronWebOptions.abi, contractAddress)
  ...
  async approveMintableERC1155(rootToken: address, options?: SendOptions) {
    ...
    const txObject = await contract.methods.setApprovalForAll(this.erc1155MintablePredicate, true)
    const web3Options = await this.web3Client.fillOptions(txObject, true /* onRootChain */, options)
    if (web3Options.encodeAbi) {
      return Object.assign(web3Options, { data: txObject.encodeABI(), to: rootToken })
    }
